...
namespace ProudNet.Handlers
{
    internal class SessionHandler : ChannelHandlerAdapter
    {
        ...
        public override async void ChannelActive(IChannelHandlerContext context)
        {
            ...
            await session.SendAsync(new NotifyServerConnectionHintMessage(config)); //client connected, send first packet, server information and parameters for client
            ...
            using (var cts = new CancellationTokenSource(_server.Configuration.ConnectTimeout))
            {
                ...
                catch (OperationCanceledException)
                {
                    ...
                    await session.SendAsync(new ConnectServerTimedoutMessage()); //client didnt respond but is connected, took too long and gets a noti for this
